...
{
    public BorderHandler(RevampHCF plugin) {
        super(plugin);
        Bukkit.getPluginManager().registerEvents(this, plugin);
    }
    
    public static boolean isWithinBorder(Location location) {
        int borderSize = RevampHCF.getInstance().getConfiguration().getBorderSizes().get(location.getWorld().getEnvironment());
    ...
    public void onPlayerPortal(PlayerPortalEvent event) {
        ...
        if (!isWithinBorder(to)) {
            ...
            if (cause != PlayerTeleportEvent.TeleportCause.NETHER_PORTAL || (cause == PlayerTeleportEvent.TeleportCause.ENDER_PEARL && isWithinBorder(event.getFrom()))) {
                ...
                if (toEnvironment != World.Environment.NORMAL) return;
                int x = to.getBlockX();
                int z = to.getBlockZ();
                int borderSize = RevampHCF.getInstance().getConfiguration().getBorderSizes().get(toEnvironment);
