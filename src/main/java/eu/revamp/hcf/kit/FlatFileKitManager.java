...
import eu.revamp.hcf.kit.event.KitRenameEvent;
...
public class FlatFileKitManager implements KitManager, Listener {
    ...
    public FlatFileKitManager(RevampHCF plugin) {
        this.plugin = plugin;
        ...
        for (Kit kit : this.kits) {
            ...
            if (kitPermission != null && !player.hasPermission(kitPermission)) {
                ...
                if (cloned != Integer.MAX_VALUE) {
                    lore.add(ChatColor.YELLOW + "Used " + factionMember.getKitUses(kit) + '/' + cloned + " times.");
                }
                if (description != null) {
                    lore.add(" ");
                    String[] wordWrap;
    ...
    public void reloadKitData() {
        this.config = new ConfigFile(this.plugin, "kits.yml");
    ...
    public void saveKitData() {
        this.config.set("kits", new LinkedList<>(this.kits));
        this.config.save();
    }
}
